...
Coverage is generated by Vitest with the V8 provider. The project enforces thresholds for statements, branches, functions, and lines in `vitest.config.ts`. The generated HTML report is ignored by Git and published at `/coverage/` inside the GitHub Pages artifact.
